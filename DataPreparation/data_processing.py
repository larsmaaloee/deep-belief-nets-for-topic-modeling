...
class DataProcessing:
    ...
    def __init__(self, paths, words_count = 2000, batchsize = 100, trainingset_size = None, trainingset_attributes = None, acceptance_lst_path = None):
    ...
    def __read_docs_from_filesystem(self):
        ...
        for i in range(number_of_batches):
            ...
            while True:
                if (len(batch_class_indices) == self.batchsize) and (not doc_count-count < self.batchsize) or (count == doc_count):
                        break
        ...
        for i in xrange(len(class_indices)):
            if not count == 0 and (count % self.batchsize) == 0: # Save the batch if batchsize is reached or if the last document has been read.
