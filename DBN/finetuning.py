...
class FineTuning:
    ...
    def run_finetuning(self,epochs):
        ...
        for epoch in range(epochs):
            ...
            evaluations.append((self.weight_matrices_added_biases,epoch,False,data_processing.get_batch_list(training=False),result_queue,self.binary_output))
            p = Pool(6)
        ...
        for largebatch in self.large_batches_lst:
            ...
            gaussian_noise = random.normal(0,4,(len(x),
                                                weight_matrices_added_biases[
                                                    (len(weight_matrices_added_biases)/2)-1].shape[1])) # Gaussian noise matrix variance 16 and mean 0.
    ...
    def get_grad_and_error(self,weights,weight_sizes,x):
        ...
        if self.binary_output:
            xout, z_values = generate_output_data(x, weights,binary_output=self.binary_output,sampled_noise=self.current_sampled_noise)
        else:
