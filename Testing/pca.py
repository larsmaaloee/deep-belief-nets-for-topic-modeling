    ...
    for i in range(int(N1)):
        for j in xrange(i+1,i+1+int(N2),1):
            ...
            for c in range(C):
                ...
                ax.plot(Z[class_mask,i], Z[class_mask,j],linestyle = 'None',alpha = 0.6,marker = markers[c], markersize = 6)
...
def pca_2d_for_2_components(X,component1,component2,class_indices,path,name,data_legend):
    ...
    for c in range(C):
        # select indices belonging to class c:
        class_mask = y.A.ravel()==c
        ax.plot(Z[class_mask,component1-1], Z[class_mask,component2-1],linestyle = 'None',alpha = 1.0,marker = markers[c], markersize = 6)
